...
model_path = 'fasterrcnn_resnet50_fpn2.pth'  #'fasterrcnn_resnet50_fpn2.pth' # Path for the best model to be saved
...
class WheatDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        records = self.df[
            self.df['image_id'] ==
            image_id]  #Getting all coordinates for the given image
...
for epoch in range(num_epochs):
    ...
    print(f"Epoch #{epoch+1} loss: {loss_hist.value}",
          "Validation IOU: {0:.4f}".format(val_iou), "Time taken :",
    ...
    if val_iou >= best_val:
        ...
        if patience == 0:
            print(
                'Early stopping. Best Validation IOU: {:.3f}'.format(best_val))
            break

model = torch.load(model_path)

test_df = pd.DataFrame()
test_df['image_id'] = np.array(
    [path.split('/')[-1][:-4] for path in glob(f'{DIR_TEST}/*.jpg')])
...
for i in range(max(10, len(test_images))):
...
#     scores = outputs[i]['scores']
#     boxes = boxes[scores >= detection_threshold].astype(np.int32)
#     image_id = test_df.image_id[i]

#     result = {'image_id': image_id, 'PredictionString': (boxes, scores)}
